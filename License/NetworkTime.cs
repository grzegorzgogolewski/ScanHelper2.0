...
namespace License
{
    public class NetworkTime
    {
        ...
        private const int TimesForEachServer = 1;
        private const byte OffTime = 40;

        private int _lastSrv = -1;
        ...
        {
            _lastSrv = (_lastSrv + 1) % Srvs.Length;
        ...
        public DateTime GetDateTime(bool utc = false)
        {
            for (int st = 0; st < Srvs.Length * TimesForEachServer; st++)
            {
                ...
                {
                    LogFile.SaveMessage($"NTP error: {Srvs[_lastSrv]} [{e.Message}]");
                }
            }

            LogFile.SaveMessage(@"NTP time = LOCAL TIME!");
            return DateTime.Now;
        }
    }
}
